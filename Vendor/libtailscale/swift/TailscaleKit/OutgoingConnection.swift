...
public actor OutgoingConnection {
    ...
    /// Local addition (imaged): hand the raw connection fd to the caller,
    /// transferring ownership — the connection no longer closes it on
    /// deinit. Used to bridge local TCP clients onto the tailnet with
    /// plain descriptor pumps (bypassing URLSession proxy quirks).
