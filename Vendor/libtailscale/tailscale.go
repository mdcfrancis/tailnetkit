...
func TsnetStart(sd C.int) C.int {
	...
	// Local addition (imaged): tsnet uses a userspace netstack with a
	// no-op tun, so binding dials to the default physical interface (a
	// loop-prevention measure for real tunnels) serves no purpose here —
	// and it bypasses VPN/filter tunnels like Cloudflare WARP, whose
	// firewalls then refuse the control-plane dials. Route normally.
